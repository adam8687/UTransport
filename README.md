...
|  Demo only | Driver route is a hardcoded simulation; replacing the interval with a Firestore geo-update listener would enable real GPS tracking |
 Auth uses a Firestore email-lookup instead of Firebase Authentication; production would use Firebase Auth with proper ID tokens | Ride history pagination and analytics dashboard for dispatchers |
 Integration with UT's official DAR (Disability Accommodation Resources) system | Driver-side app and two-way ETA updates |
